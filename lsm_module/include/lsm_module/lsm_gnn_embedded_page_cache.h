#ifndef __LSM_GNN_EMBEDDED_PAGE_CACHE_H__
#define __LSM_GNN_EMBEDDED_PAGE_CACHE_H__
/*
 * LSM-GNN NVMe: self-contained extended BAM page cache (wb_bam_ptr, next_reuse,
 * reuse_chunk, PVP / is_pvp, extended page_cache_t ctor). Vendored from LSM-GNN/bam so
 * LSM_NVMe can build against a slim upstream page_cache.h without pulling GNN
 * patches. Include only via lsm_gnn_page_cache.h; do not mix with another
 * page_cache.h in the same translation unit.
 */
...
/* __match_any_sync is declared via cuda_runtime (sm_70_rt). For sm < 70, emulate. */
#ifdef __CUDACC__
#include <cuda_runtime.h>
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 700
template <typename T>
__device__ __forceinline__ unsigned int bam_match_any_sync(unsigned int mask,
                                                           T value) {
...
#pragma unroll
    for (int lane = 0; lane < 32; ++lane) {
        if ((mask & (1u << lane)) == 0)
            continue;
        T peer = __shfl_sync(mask, value, lane);
        if (peer == value)
            result |= (1u << lane);
    }
    return result;
}
#define BAM_MATCH_ANY_SYNC(mask, val) bam_match_any_sync((mask), (val))
#else
#define BAM_MATCH_ANY_SYNC(mask, val) __match_any_sync((mask), (val))
#endif
#else
#define BAM_MATCH_ANY_SYNC(mask, val) __match_any_sync((mask), (val))
#endif
...
#include "lsm_gnn_window_buffer.h"
...
struct tlb {
    ...
    T* acquire(const size_t i, const size_t gid, size_t& start, size_t& end, range_d_t<T>* range, const size_t page_) {
        ...
        uint32_t eq_mask = BAM_MATCH_ANY_SYNC(mask, gid);
        eq_mask &= BAM_MATCH_ANY_SYNC(mask, (uint64_t)this);
...
struct wb_bam_ptr {
    ...
    T* update_page(const size_t i) {
    fini(); //destructor
    addr = (T*) array->wb_acquire_page(i, page, start, end, range_id, time_step, head_ptr);
     return addr;
    }
...
struct page_cache_d_t {
    ...
    /** Total NVMe read commands (primary ``read_data`` + optional second in ``enqueue_second``). */
    simt::atomic<uint64_t, simt::thread_scope_device>* ssd_read_ops;
    ...
    // PVP (prefetch-aware eviction / CPU aggregation path)
    bool is_pvp;

    uint64_t* pvp_meta_queue;
    uint64_t* pvp_loc_queue;
    uint32_t pvp_buffer_depth;
    uint32_t* pvp_queue_counter;

    /** Pinned host-mapped eviction staging (set by host after pinned_eviction_buffer_init). */
    uint32_t* pinned_evict_counter;
    void* pinned_evict_queue;
    uint64_t* pinned_evict_ids;
    uint32_t pinned_evict_num_buffers;
    uint32_t pinned_evict_queue_depth;
    ...
    __forceinline__
    __device__
    uint32_t wb_find_slot_pvp(uint64_t address, uint64_t range_id, const uint32_t queue_,  simt::atomic<uint64_t, simt::thread_scope_device>& access_cnt, uint64_t* evicted_p_array, 
                         int& evict_cpu, uint32_t& evicted_page_id, uint32_t& queue_reuse,
                         uint8_t time_step, uint32_t head_ptr, uint32_t& evict_time);
...
struct page_cache_t {
    ...
    BufferPtr ssd_read_ops_buf;

    /** Prefetch-aware eviction (PVP): extra host-mapped queues + per-head counters. */
    bool is_pvp = false;
    uint32_t num_pvp_buffers = 0;
    uint32_t pvp_buffer_depth = 0;

    uint64_t* h_pvp_meta_queue = nullptr;
    uint64_t* h_pvp_loc_queue = nullptr;
    uint64_t* pvp_meta_queue = nullptr;
    uint64_t* pvp_loc_queue = nullptr;
    ...
    void print_reset_stats(void) {
        ...
        uint64_t n_ssd_reads = 0;
        if (pdt.ssd_read_ops != nullptr) {
            cuda_err_chk(cudaMemcpy(
                &n_ssd_reads, pdt.ssd_read_ops,
                sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaMemcpyDeviceToHost));
            std::cout << "ssd_nvme_read_ops: " << std::dec << n_ssd_reads << std::endl;
            cuda_err_chk(cudaMemset(
                pdt.ssd_read_ops, 0,
    ...
    /** Device staging for victim feature copies; must match pinned_eviction_buffer geometry. */
    void bind_pinned_eviction_staging(uint32_t* counter, void* queue_dev, uint64_t* ids,
                                      uint32_t num_buffers, uint32_t queue_depth) {
        pdt.pinned_evict_counter = counter;
        pdt.pinned_evict_queue = queue_dev;
        pdt.pinned_evict_ids = ids;
        pdt.pinned_evict_num_buffers = num_buffers;
        pdt.pinned_evict_queue_depth = queue_depth;
    ...
    page_cache_t(const uint64_t ps, const uint64_t np, const uint32_t cudaDevice,
                 const Controller& ctrl, const uint64_t max_range,
                 const std::vector<Controller*>& ctrls, bool is_pvp_enable = false,
                 uint32_t num_pvp_buffers_in = 256, uint32_t pvp_buffer_depth_in = 0) {
        this->is_pvp = is_pvp_enable;
        this->num_pvp_buffers = num_pvp_buffers_in;
        this->pvp_buffer_depth = is_pvp_enable ? pvp_buffer_depth_in : 0u;

        if (is_pvp_enable) {
            cuda_err_chk(cudaMalloc(reinterpret_cast<void**>(&pvp_queue_counter),
                                    sizeof(uint32_t) * num_pvp_buffers_in));
            if (pvp_buffer_depth_in > 0) {
                const size_t ring_bytes =
                    sizeof(uint64_t) * static_cast<size_t>(num_pvp_buffers_in) *
                    static_cast<size_t>(pvp_buffer_depth_in);
                cuda_err_chk(
                    cudaHostAlloc(reinterpret_cast<void**>(&h_pvp_meta_queue), ring_bytes,
                                  cudaHostAllocMapped));
                cuda_err_chk(
                    cudaHostAlloc(reinterpret_cast<void**>(&h_pvp_loc_queue), ring_bytes,
                                  cudaHostAllocMapped));
                cudaHostGetDevicePointer(reinterpret_cast<void**>(&pvp_meta_queue),
                                         reinterpret_cast<void*>(h_pvp_meta_queue), 0);
                cudaHostGetDevicePointer(reinterpret_cast<void**>(&pvp_loc_queue),
                                         reinterpret_cast<void*>(h_pvp_loc_queue), 0);
            }
        }

        pdt.pvp_loc_queue = pvp_loc_queue;
        pdt.pvp_meta_queue = pvp_meta_queue;
        pdt.pvp_buffer_depth = this->pvp_buffer_depth;
        pdt.pvp_queue_counter = pvp_queue_counter;
        pdt.is_pvp = is_pvp_enable;
        pdt.pinned_evict_counter = nullptr;
        pdt.pinned_evict_queue = nullptr;
        pdt.pinned_evict_ids = nullptr;
        pdt.pinned_evict_num_buffers = 0;
        pdt.pinned_evict_queue_depth = 0;
        ...
        ssd_read_ops_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        ...
        pdt.ssd_read_ops = (simt::atomic<uint64_t, simt::thread_scope_device>*)ssd_read_ops_buf.get();
        cuda_err_chk(cudaMemset(
            pdt.ssd_read_ops, 0,
            sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
...
struct range_t {
    ...
    /** Copy ``c_h->pdt`` into embedded ``rdt.cache`` and upload (after page_cache_t updates e.g. pinned staging). */
    void sync_embedded_page_cache_from(page_cache_t* c_h);
};
...
template <typename T>
void range_t<T>::sync_embedded_page_cache_from(page_cache_t* c_h) {
    rdt.cache = c_h->pdt;
    cuda_err_chk(cudaMemcpy(d_range_ptr, &rdt, sizeof(range_d_t<T>), cudaMemcpyHostToDevice));
}
    ...
    do {
	    ...
	    uint32_t evict_time = 65535;
            if ((st_new & BUSY) == 0) {
                ...
                uint32_t page_trans;
                if(cache.is_pvp){
                    page_trans = cache.wb_find_slot_pvp(index, range_id, queue, read_io_cnt, evicted_p_array,
...
//                //printf("++tid: %llu\tbase: %p  page:%llu\n", (unsigned long long) threadIdx.x, base_master, (unsigned long long) page);
    ...
    void coalesce_page(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write,
        ...
        eq_mask = BAM_MATCH_ANY_SYNC(mask, gaddr);
        eq_mask &= BAM_MATCH_ANY_SYNC(mask, (uint64_t)this);
...
//                //printf("++tid: %llu\tbase: %p  page:%llu\n", (unsigned long long) threadIdx.x, base_master, (unsigned long long) page);
    ...
    void wb_coalesce_page(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write, uint32_t& eq_mask, int& master, uint32_t& count, uint64_t& base_master, 
        ...
        if (master == lane) {
            base = r_->wb_acquire_page(page, count, dirty, ctrl, queue, 
                                      evict_cpu, queue_idx, queue_reuse, time_step, head_ptr);
            page_trans = base;
            base_master = base;
        }
        base_master = __shfl_sync(eq_mask,  base_master, master);
        ...
        if(evict_cpu){
            ...
            page_cache_d_t& pcd = r_->cache;
            void* qbase = pcd.pinned_evict_queue;
            const uint32_t qd = pcd.pinned_evict_queue_depth;
            write_to_queue<uint64_t>(cache_ptr,
                (void*)((uint8_t*)qbase + (static_cast<size_t>(qd) * queue_reuse * pcd.page_size) +
                        (static_cast<size_t>(queue_idx) * pcd.page_size)),
                (pcd.page_size), eq_mask);
            
            __syncwarp(eq_mask);
            if (master == lane) {
                base = r_->wb_cont_acquire_page(page, count, dirty, ctrl, queue, page_trans);
               base_master = base;
            }
            base_master = __shfl_sync(eq_mask,  base_master, master);
        }
    ...
    void* acquire_page(const size_t i, data_page_t*& page_, size_t& start, size_t& end, int64_t& r) const {
        ...
        __forceinline__
    __device__
    void* wb_acquire_page(const size_t i, data_page_t*& page_, size_t& start, size_t& end, int64_t& r,
                          uint8_t time_step, uint32_t head_ptr) const {
        uint32_t lane = lane_id();
        r = find_range(i);
        auto r_ = d_ranges+r;
 ...
 //      	r_->cache.cache_pages[page_trans].next_reuse = reuse_time;
    ...
    T AtomicAdd(const size_t i, const T val) const {
        ...
        if (r != -1) {
            ...
            uint32_t active_cnt = __popc(mask);
            uint32_t eq_mask = BAM_MATCH_ANY_SYNC(mask, gaddr);
            eq_mask &= BAM_MATCH_ANY_SYNC(mask, (uint64_t)this);
...
//	    }
    ...
    /**
     * ``array_d_t`` holds a device copy of each ``range_d_t`` taken at construction time.
     * After mutating the canonical range on ``range_t::d_range_ptr`` (e.g. PVP staging bind
     * + :func:`range_t::sync_embedded_page_cache_from`), refresh this duplicate so kernels
     * that use ``array_d_t::d_ranges`` see the updated embedded ``page_cache_d_t``.
     */
    void resync_device_range_from(const range_t<T>& r, size_t k = 0) {
        if (k >= adt.n_ranges || adt.d_ranges == nullptr || r.d_range_ptr == nullptr)
            return;
        cuda_err_chk(cudaMemcpy(adt.d_ranges + k, r.d_range_ptr, sizeof(range_d_t<T>),
                    ...
                    if (((new_expected_state & BUSY ) == 0) ) {
			            ...
			            if (((new_expected_state & CNT_MASK ) == 0) ) {                         
                           ...
                           // //printf("CHECK REUSE prev_ page: %lu reuse val: %u \n", (unsigned long) prev_page, (unsigned) reuse_val );
                            ...
                            if(reuse_val < 65535){
                                const uint32_t nbuf = pinned_evict_num_buffers;
                                const uint32_t qdep = pinned_evict_queue_depth;
                                // PVP / pinned-evict ring index = reuse_val (high bits of next_reuse) mod nbuf.
                                // time_step / head_ptr are unused here; reuse_val is driven by prefetch_ts
                                // and any host time-step policy.
                                (void)time_step;
                                (void)head_ptr;
                                uint32_t cur_head = static_cast<uint32_t>(
                                    reuse_val % static_cast<uint64_t>(nbuf));

                                uint32_t queue_counter = atomicAdd(pinned_evict_counter+cur_head, 1);
                                evict_time = reuse_val;
                                if(queue_counter < qdep){
                                        ...
                                        pinned_evict_ids[queue_idx + qdep * queue_reuse] = (reuse_full & (0x0000FFFFFFFFFFFF));
...
__forceinline__
__device__
        uint32_t page_cache_d_t::wb_find_slot_pvp(uint64_t address, uint64_t range_id, const uint32_t queue_, simt::atomic<uint64_t, simt::thread_scope_device>& access_cnt, uint64_t* evicted_p_array,
                                      int& evict_cpu, uint32_t& queue_idx, uint32_t& queue_reuse,
                                      uint8_t time_step, uint32_t head_ptr, uint32_t& evict_time) {
    bool fail = true;
    uint64_t count = 0;
                                ...
                                if(queue_counter < qdep){
                                    ...
                                    for(int sec = 0; sec < 8; sec++){
                                        ...
                                        if(reuse_sec_val != reuse_val){
                                            uint32_t sec_head_ptr = static_cast<uint32_t>(
                                                reuse_sec_val % static_cast<uint64_t>(nbuf));
                                            if(reuse_sec_val < 65535){
                                                uint32_t cpu_queue_counter = atomicAdd(pvp_queue_counter+sec_head_ptr, 1);
                                                if(cpu_queue_counter < pvp_buffer_depth){
                                                    ...
                                                    pvp_meta_queue[cpu_queue_counter + sec_head_ptr * pvp_buffer_depth] = memcpy_ptr;
                                                    pvp_loc_queue[cpu_queue_counter + sec_head_ptr * pvp_buffer_depth] = (reuse_sec & (0x0000FFFFFFFFFFFF));
                                                }
                                            }
                                        }
                                    }                                     
...
inline __device__ void read_data(page_cache_d_t* pc, QueuePair* qp, const uint64_t starting_lba, const uint64_t n_blocks, const unsigned long long pc_entry) {
    ...
    pc->ssd_read_ops->fetch_add(1, simt::memory_order_relaxed);
...
#endif // __LSM_GNN_EMBEDDED_PAGE_CACHE_H__
